...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				...
				// 检查 beanName 对应的 BeanDefinition 是否已经注册，如果没有注册，则注册
	 ...
	 * @return {@code true} if the bean can be registered as-is; true: 未注册 false: 已经注册
	...
	protected boolean checkCandidate(String beanName, BeanDefinition beanDefinition) throws IllegalStateException {
		...
		// map 中没有该 beanName，则返回 true，未注册
		if (!this.registry.containsBeanDefinition(beanName)) {
			return true;
		}

		// 根据 beanName 获取容器中的 BeanDefinition
		BeanDefinition existingDef = this.registry.getBeanDefinition(beanName);
		BeanDefinition originatingDef = existingDef.getOriginatingBeanDefinition();
		if (originatingDef != null) {
			existingDef = originatingDef;
		}
		// 已经存在的 beanDefinition 与当前的是否一样的，如果一样，说明已经存在，不需要注册了
	...
	protected boolean isCompatible(BeanDefinition newDefinition, BeanDefinition existingDefinition) {
		// 已经存在的 beanDefinition 与当前的对应的 source 一样
